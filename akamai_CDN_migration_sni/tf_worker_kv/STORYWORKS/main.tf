...
resource "cloudflare_workers_kv_namespace" "storyworks" {
  title = var.kv.title
}

resource "cloudflare_workers_kv" "storyworks" {
  count        = length(var.kv.list)
  namespace_id = cloudflare_workers_kv_namespace.storyworks.id
